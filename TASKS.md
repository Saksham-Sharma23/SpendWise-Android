...
| ---------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-22 | **Liquid glass = the reference shader's optics, evaluated around the rim; no refraction**                           | Its Fresnel ramp and two-lobe glare port exactly (`lib/glassOptics.ts`); the glare drifts with scroll. Bending the background needs a shader reading the pixels behind the bar — an AGSL native module, Android 13+ — so it was not built. Nearly clear (3 px blur, no veil). A highlight that followed the finger was built and removed: distracting             |
| 2026-09-22 | **Settings → Glass effect: Frosted (default) or Liquid**                                                            | A STYLE choice, not the "Reduce transparency" perf fallback rejected 2026-09-19 (R5-4: blur costs nothing measurable). Liquid needs a real blur, so below Android 12 it is shown but disabled                                                                                                                                                                     |
| 2026-09-22 | **Forms rise and fade (`fade_from_bottom`), not the full-height slide**                                             | Android's stack animations are fixed; the slide ran ~400 ms on an untunable curve. Still from below, settled at ~200 ms                                                                                                                                                                                                                                           |
| 2026-09-22 | **Tabs cross-fade (180 ms, native driver) and are preloaded after launch**                                          | Switching cut instantly, and a first visit built the tab on screen. Tab-icon pop and Home trend bars stay bouncy, by choice                                                                                                                                                                                                                                       |
| 2026-09-22 | **Deferred keyboard focus waits for the stack's `transitionEnd`**                                                   | `InteractionManager.runAfterInteractions` is a one-tick stub in RN 0.86; the keyboard still rose mid-slide                                                                                                                                                                                                                                                        |
| 2026-09-22 | **Liquid glass gets real refraction on Android 13+: a local native module (`modules/liquid-glass`)**                | Reverses the "no refraction" row above: the look wanted is a lens, and drawn light cannot move pixels. The screen is recorded on the GPU (as expo-blur does) and drawn through an AGSL shader: swell across the short axis, a quarter-circle bend at the rim, dispersion. Needs a native rebuild. Android 12 keeps the drawn liquid glass; below 12, frosted only |
