import type { SystemCategory } from './seedCore';

/**
 * The built-in categories, as plain data.
 *
 * Kept apart from db/seed.ts, which reaches the native handle through
 * db/client and so cannot be loaded in Node. Here, a test can import the list
 * directly — lib/__tests__/icons.test.ts checks every icon is a real one.
 *
 * Identity is the fixed `uid` (`sys:rent`), never the name (db/seedCore.ts).
 * Migration 0003 gave existing system rows these uids. Icons are lucide
 * names, matching the web app.
 *
