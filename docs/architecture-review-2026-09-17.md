...
| **B10** | `lib/categoryColor.ts:14` + `Ledger.tsx:58`       | The uncategorised fallback returns `colors.muted`, the text colour of the _current theme_, and it is baked into row objects when they are fetched.                                                        | Switch theme → uncategorised rows keep the previous theme's grey.                        |
...
shared _domain_ queries, so they are copied:
...
B1 and B2 exist _because_ of this duplication: two copies of "this month's expenses" drifted apart.
...
| Area                                         | State                                                                                                                                                 |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phases 0–5, Groups                           | Code complete. **On-device verification outstanding for most of them** (see TASKS.md → _Device verification_)                                         |
