...
import { CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Minus, Plus, Trash2, Users, X } from 'lucide-react-native';
...
export function ExpenseForm({ categories }: { categories: CategoryOption[] }) {
  ...
  const [draft, setDraft] = useState<Draft>(() =>
    existing
      ? draftFromExpense(existing, membersFor(initialTarget, selfId).map((m) => m.id), selfId, paiseToDecimalString, formatPercent)
      : emptyDraft(membersFor(initialTarget, selfId).map((m) => m.id), selfId),
  );
  ...
  const chooseTarget = (next: Target) => {
    ...
    setDraft((d) => ({ ...emptyDraft(membersFor(next, selfId).map((m) => m.id), selfId), amount: d.amount }));
    setSheet(null);
  };
  ...
  const paidLabel = draft.paidMode === 'multiple' ? `${result.payers?.length ?? 'several'} people` : nameOf(draft.payerId);
      ...
      <KeyboardAvoidingView behavior="padding" className="flex-1" style={{ paddingTop: insets.top }}>
        ...
        <ScrollView className="px-5" keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 24 }}>
          ...
          <Animated.View entering={FadeInDown.delay(50).duration(300)} className="mt-6 flex-row items-center gap-3">
            ...
            <TextInput
              ...
              style={{ color: colors.foreground, fontFamily: fonts.semibold, fontSize: 18, borderColor: colors.borderStrong }}
            />
          </Animated.View>
          <Animated.View entering={FadeInDown.delay(100).duration(300)} className="mt-3 flex-row items-center gap-3">
            ...
            <TextInput
              ...
              style={{ color: colors.foreground, fontFamily: fonts.bold, fontSize: 38, letterSpacing: -1, borderColor: colors.borderStrong, fontVariant: ['tabular-nums'] }}
            />
          </Animated.View>
            ...
            <View className="flex-row items-center gap-2">
              ...
              <View className="flex-1 flex-row items-center justify-end gap-1">
                ...
                <View className="flex-row items-center gap-1.5 px-1">
                  ...
                  <Text style={{ color: colors.foreground, fontFamily: fonts.semibold, fontSize: 13 }}>{formatDayMonth(date)}</Text>
                </View>
              ...
              {categories.map((c) => {
                ...
                return (
                  <PressableScale
                    ...
                    style={{ borderColor: on ? t : colors.border, backgroundColor: on ? withAlpha(t, 0.16) : colors.card }}
                  >
                    <CategoryIcon icon={c.icon} color={t} size={24} />
                    <Text style={{ color: on ? colors.foreground : colors.muted, fontFamily: on ? fonts.semibold : fonts.medium, fontSize: 13 }}>{c.name}</Text>
                  </PressableScale>
                );
              })}
            </ScrollView>
          ...
          <TextInput
            ...
            style={{ color: colors.foreground, fontFamily: fonts.medium, fontSize: 14, backgroundColor: colors.card, borderColor: colors.border, minHeight: 52 }}
          />
        </ScrollView>
        ...
        <View className="px-5 pt-3" style={{ paddingBottom: insets.bottom + 12, borderTopWidth: 1, borderTopColor: colors.border }}>
          {!canSave && target != null && draft.amount.trim() !== '' && result.problem ? (
            <Text style={{ color: colors.expense, fontFamily: fonts.medium, fontSize: 12, textAlign: 'center', marginBottom: 8 }}>{result.problem}</Text>
          ) : null}
          <PressableScale
            accessibilityRole="button"
            onPress={onSave}
          ...
          >
            <Text style={{ color: colors.onPrimary, fontFamily: fonts.bold, fontSize: 16 }}>{editingId != null ? 'Save changes' : 'Save expense'}</Text>
          </PressableScale>
        </View>
      </KeyboardAvoidingView>

      <TargetSheet visible={sheet === 'target'} hub={hub} current={target} onPick={chooseTarget} onClose={() => (target ? setSheet(null) : router.back())} />
      <PaidSheet visible={sheet === 'paid'} draft={draft} setDraft={setDraft} members={members} result={result} onClose={() => setSheet(null)} />
      <SplitSheet visible={sheet === 'split'} draft={draft} setDraft={setDraft} members={members} result={result} onClose={() => setSheet(null)} />
    </View>
  );
}
...
function Pill({ label, onPress, disabled = false }: { label: string; onPress: () => void; disabled?: boolean }) {
  ...
  return (
    ...
    >
      <Text numberOfLines={1} style={{ color: colors.primary, fontFamily: fonts.semibold, fontSize: 15, maxWidth: 200 }}>
        {label}
      </Text>
...
function SplitSummary({ result, draft, members, selfId }: { result: ReturnType<typeof evaluate>; draft: Draft; members: Member[]; selfId: number }) {
  const colors = useColors();
  if (result.amountPaise == null) return null;
  if (result.problem) {
    return <Text style={{ color: colors.warning, fontFamily: fonts.medium, fontSize: 12, marginTop: 8 }}>{result.problem}</Text>;
  }
  let text: string;
  if (draft.method === 'equal' && result.shares) {
    ...
    text = (result.shares ?? [])
      .map((s) => `${s.personId === selfId ? 'you' : (members.find((m) => m.id === s.personId)?.name ?? '?')} ${formatINR(s.paise, { whole: s.paise % 100 === 0 })}`)
      .join(' · ');
  }
  return (
    <Text numberOfLines={2} style={{ color: colors.muted, fontFamily: fonts.regular, fontSize: 12, marginTop: 8, textAlign: 'center' }}>
      {text}
    </Text>
  );
}
    ...
    <FormSheet visible={visible} title="Split with" onClose={onClose}>
      {hub.groups.length === 0 && hub.friends.length === 0 ? (
        <View className="items-center py-6">
          ...
          <Text style={{ color: colors.muted, fontFamily: fonts.medium, fontSize: 14, marginTop: 8, textAlign: 'center' }}>
            Create a group or add a friend first.
          </Text>
          <PressableScale accessibilityRole="button" onPress={() => router.replace('/(modals)/group')} className="mt-4 rounded-full px-5 py-3" style={{ backgroundColor: colors.primary }}>
      ...
      {hub.groups.map((g) => {
        ...
        return (
          <OptionRow
            ...
            leading={<CategoryIcon icon={g.icon ?? deterministicIcon(g.name)} color={deterministicColor(g.name)} size={38} />}
            title={g.name}
...
function OptionRow({ on, onPress, leading, title, subtitle }: { on: boolean; onPress: () => void; leading: React.ReactNode; title: string; subtitle?: string }) {
  const colors = useColors();
  return (
    <PressableScale
    ...
    >
      ...
      <View className="flex-1">
        ...
        {subtitle ? <Text style={{ color: colors.muted, fontFamily: fonts.regular, fontSize: 12 }}>{subtitle}</Text> : null}
      </View>
...
function RemainingFooter({ remaining, kind, total }: { remaining: number; kind: 'paise' | 'bp'; total: number | null }) {
  const colors = useColors();
  ...
  return (
    <View className="items-center rounded-2xl py-3" style={{ backgroundColor: ok ? withAlpha(colors.income, 0.12) : withAlpha(colors.expense, 0.1) }}>
...
function PaidSheet({
  ...
  return (
    <FormSheet
      ...
      title="Who paid?"
      onClose={onClose}
      footer={draft.paidMode === 'multiple' ? <RemainingFooter remaining={result.paidRemaining} kind="paise" total={result.amountPaise} /> : undefined}
    >
      {members.map((m) => (
        <OptionRow
          key={m.id}
      ...
      {members.length > 1 ? (
        <OptionRow
          ...
          leading={
            <View className="items-center justify-center rounded-full" style={{ width: 36, height: 36, backgroundColor: colors.elevated }}>
...
function SplitSheet({
  ...
  const colors = useColors();
  const shareOf = useCallback((id: number) => result.shares?.find((s) => s.personId === id)?.paise ?? 0, [result.shares]);
  ...
  return (
    <FormSheet visible={visible} title="Split" onClose={onClose} footer={footer}>
      ...
      {result.amountPaise == null ? (
        <Text style={{ color: colors.warning, fontFamily: fonts.medium, fontSize: 13, marginTop: 14, textAlign: 'center' }}>Enter the amount first</Text>
      ) : null}
      ...
      <View className="mt-4 gap-2">
        {members.map((m) => {
          if (draft.method === 'equal') {
            ...
            return (
              ...
              >
                ...
                <Text style={{ color: on ? colors.foreground : colors.subtle, fontFamily: fonts.semibold, fontSize: 14, fontVariant: ['tabular-nums'] }}>
                  {on ? formatINR(shareOf(m.id)) : '—'}
                </Text>
                <View
                  className="items-center justify-center rounded-lg"
                  style={{ width: 24, height: 24, backgroundColor: on ? colors.primary : 'transparent', borderWidth: on ? 0 : 1.5, borderColor: colors.borderStrong }}
                >
          ...
          if (draft.method === 'exact') {
            return (
              <AmountRow key={m.id} member={m} value={draft.exact[m.id] ?? ''} prefix="₹" onChange={(v) => setDraft((d) => ({ ...d, exact: { ...d.exact, [m.id]: v } }))} />
            );
          }
          ...
          const set = (n: number) => setDraft((d) => ({ ...d, shares: { ...d.shares, [m.id]: String(Math.max(0, Math.min(99, n))) } }));
          return (
            <View key={m.id} className="flex-row items-center gap-3 rounded-2xl px-3 py-2" style={{ backgroundColor: colors.card }}>
              <Avatar name={m.name} isSelf={m.isSelf} size={36} />
              <View className="flex-1">
                <Text style={{ color: colors.foreground, fontFamily: fonts.medium, fontSize: 15 }}>{m.isSelf ? 'You' : m.name}</Text>
                <Text style={{ color: colors.muted, fontFamily: fonts.regular, fontSize: 12 }}>{formatINR(shareOf(m.id))}</Text>
              </View>
              <RoundButton label={`Fewer shares for ${m.name}`} onPress={() => set(units - 1)}>
                <Minus size={16} color={colors.foreground} />
              </RoundButton>
              <Text style={{ color: colors.foreground, fontFamily: fonts.bold, fontSize: 17, minWidth: 24, textAlign: 'center' }}>{units}</Text>
...
function AmountRow({
  ...
  const colors = useColors();
  const invalid = value.trim() !== '' && (suffix ? !/^\d{0,3}(\.\d{0,2})?%?$/.test(value.trim()) : parseAmountToPaise(value) == null);
  return (
    <View className="flex-row items-center gap-3 rounded-2xl px-3 py-2" style={{ backgroundColor: colors.card }}>
      ...
      <View className="flex-1">
        <Text style={{ color: colors.foreground, fontFamily: fonts.medium, fontSize: 15 }}>{member.isSelf ? 'You' : member.name}</Text>
      ...
      <View
        ...
        style={{ borderColor: invalid ? colors.expense : colors.border, backgroundColor: colors.background, minWidth: 110 }}
      >
        {prefix ? <Text style={{ color: colors.muted, fontFamily: fonts.semibold, fontSize: 15 }}>{prefix}</Text> : null}
        <TextInput
          value={value}
          ...
          style={{ flex: 1, color: colors.foreground, fontFamily: fonts.semibold, fontSize: 16, paddingVertical: 8, textAlign: 'right', fontVariant: ['tabular-nums'] }}
        />
        {suffix ? <Text style={{ color: colors.muted, fontFamily: fonts.semibold, fontSize: 15 }}>{suffix}</Text> : null}
      </View>
    </View>
  );
}
