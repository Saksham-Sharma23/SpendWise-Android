...
export function GroupTotals({ groupId }: { groupId: number }) {
  ...
  if (group.status === 'pending' || stats.status === 'pending') return <Screen back title="Totals">{null}</Screen>;
  if (s.count === 0) {
    return (
      <Screen back title="Totals" subtitle={name}>
        <View className="px-5">
          <EmptyState icon={ChartPie} title="Nothing spent yet" description="Totals appear once the group has an expense." />
        </View>
      </Screen>
    );
  }
      ...
      <View className="gap-4 px-5" style={{ paddingBottom: 32 }}>
        <Animated.View entering={FadeInDown.duration(340)}>
          <Card variant="accent" className="p-5">
            ...
            <Text style={{ color: colors.foreground, fontFamily: fonts.bold, fontSize: 32, letterSpacing: -1, marginTop: 2 }}>
          ...
          <Card className="items-center p-5">
            ...
            <Donut slices={slices} selectedKey={selected} onSelect={setSelected}>
              <View pointerEvents="none" className="items-center px-8">
                ...
                <Text numberOfLines={1} adjustsFontSizeToFit style={{ color: colors.foreground, fontFamily: fonts.bold, fontSize: 21 }}>
              ...
              {s.categories.map((c) => {
                ...
                return (
                  <View key={key} className="flex-row items-center gap-3" style={{ opacity: selected && selected !== key ? 0.5 : 1 }}>
                    ...
                    <Text numberOfLines={1} className="flex-1" style={{ color: colors.foreground, fontFamily: fonts.medium, fontSize: 14 }}>
                  ...
                  <View key={m.personId} className="flex-row items-center gap-3">
                    ...
                    <View className="flex-1">
                      <View className="flex-row justify-between">
                        <Text style={{ color: colors.foreground, fontFamily: fonts.medium, fontSize: 14 }}>{isSelf ? 'You' : (person?.name ?? 'Someone')}</Text>
                        <Text style={{ color: colors.foreground, fontFamily: fonts.semibold, fontSize: 14, fontVariant: ['tabular-nums'] }}>
                          {formatINR(m.owedPaise, { whole: true })}
                        </Text>
                      </View>
                      <View className="mt-1.5 h-1.5 overflow-hidden rounded-full" style={{ backgroundColor: colors.elevated }}>
                        <View className="h-full rounded-full" style={{ width: `${(m.owedPaise / maxShare) * 100}%`, backgroundColor: isSelf ? colors.primary : colors.borderStrong }} />
                      </View>
                    </View>
                  </View>
                );
...
function Stat({ label, value, hint }: { label: string; value: string; hint: string }) {
  ...
  return (
    <View className="flex-1 rounded-2xl px-3 py-2.5" style={{ backgroundColor: colors.elevated }}>
      ...
      <Text numberOfLines={1} adjustsFontSizeToFit style={{ color: colors.foreground, fontFamily: fonts.bold, fontSize: 17, marginTop: 2 }}>
        {value}
      </Text>
