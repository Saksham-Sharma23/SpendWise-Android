...
export function FriendForm() {
  ...
  const duplicate = getFriends().some((f) => f.id !== editingId && f.name.trim().toLowerCase() === name.trim().toLowerCase());
  ...
  return (
    <KeyboardAvoidingView behavior="padding" className="flex-1" style={{ paddingTop: insets.top, backgroundColor: colors.background }}>
      <View className="flex-row items-center justify-between px-5 py-3">
        ...
        <Text style={{ color: colors.foreground, fontFamily: fonts.semibold, fontSize: 16 }}>{editingId != null ? 'Edit friend' : 'Add a friend'}</Text>
        {editingId != null ? (
      ...
      <View className="flex-1 items-center px-5 pt-10">
        ...
        <TextInput
          ...
          style={{ color: colors.foreground, fontFamily: fonts.semibold, fontSize: 18, backgroundColor: colors.card, borderColor: colors.border }}
        />
        {duplicate ? (
          ...
          <Text style={{ color: colors.subtle, fontFamily: fonts.regular, fontSize: 12, marginTop: 8, textAlign: 'center' }}>
      ...
      <View className="px-5 pt-3" style={{ paddingBottom: insets.bottom + 12 }}>
        <PressableScale accessibilityRole="button" onPress={onSave} className="items-center rounded-full py-4" style={{ backgroundColor: colors.primary }}>
          <Text style={{ color: colors.onPrimary, fontFamily: fonts.bold, fontSize: 16 }}>{editingId != null ? 'Save' : 'Add friend'}</Text>
        </PressableScale>
      </View>
